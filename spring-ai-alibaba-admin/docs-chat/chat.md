...
现在我已经把项目启动起来了，包括中间件也启动起来了，如果没有的话，参考 docs-research/scripts 路径下相关脚本完成启动。
现在我要求你阅读我前期所做的所有文档（docs-research/ 路径下所有文件）或者阅读项目本身源码，生成一个使用案例，要求：
- 案例完整，能覆盖项目的核心链路和功能
- 用户能方便的根据案例在前端进行操作（粘贴复制）
- 案例构建逻辑清晰，用户根据案例能清晰的理解项目的定位和逻辑
输出用一份md文档，保存到 docs-research/16-use-case.md。
